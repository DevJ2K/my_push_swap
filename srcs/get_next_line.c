...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/14 14:40:00 by tajavon           #+#    #+#             */
/*   Updated: 2024/02/07 18:38:43 by tajavon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	new_all_read(char **all_read, char *buffer, int nb_bytes_read)
{
	...
	all_read_length = 0;
	if (*all_read)
		all_read_length = ft_strlen_int(*all_read);
...
void	clean_read_line(char **all_read)
{
	...
	if ((*all_read)[j] == '\n')
		j++;
	temp = malloc((ft_strlen_int(*all_read) - j + 1) * sizeof(char));
	if (!temp)
		return ;
	i = 0;
	while ((*all_read)[j])
